...
public class Token {
  ...
  private static char[] splitter = { '.', '9', '_' };
  ...
  private List<Fragment> tokanize(final String name) {
    ...
    if (element != null) {
      ...
      for (final char next : token) {
        position++;
        if (next >= splitter[0] && next <= splitter[1]
            || next == splitter[2]) {
  ...
  public Token replaceFragments(final Map<Fragment, String> replacments) {
    ...
    for (final Fragment f : fragments)
      if (replacments.containsKey(f))
        ...
        result.append(f.getOriginalFragment());
  ...
  private String adoptReplacement(final Map<Fragment, String> replacements, final Fragment f) {
    final String replacement = replacements.get(f);
    if (f.isAllUpper())
